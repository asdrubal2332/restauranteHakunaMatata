...
<span class="hover:bg-[#393939] px-4 py-2 rounded-xl transition-colors cursor-pointer" onclick="location.href='gestion_mesas.html'">Distribución de Mesas</span>
<span class="hover:bg-[#393939] px-4 py-2 rounded-xl transition-colors cursor-pointer" onclick="location.href='toma_pedidos.html'">Pedidos</span>
<span class="text-[#FF8C00] px-4 py-2 rounded-xl transition-colors cursor-pointer">Gestión de Personal</span>
<span class="hover:bg-[#393939] px-4 py-2 rounded-xl transition-colors cursor-pointer" onclick="location.href='facturacion.html'">Configuración</span>
</div>
<div class="w-10 h-10 rounded-full bg-surface-container-high border-2 border-[#FF8C00] overflow-hidden">
<img alt="Staff Profile" class="w-full h-full object-cover" src="https://lh3.googleusercontent.com/aida-public/AB6AXuASusyeQuT060vmsoxSp0DxAxD4-Bt-KwTII6a0E7CnTTuLO61BIwMzH-P5uAWyVULjGjtQRXlc2HBGNeI10RBBkRH_u98l83BA3xIhNC0gp8FINt9xcY30ZFNzSqA7tC7YbXjKh_spczGPCLTz1q8c64qPighczj3rQBvLZ8pQxGwzFDa0ZEqu4w0zGjYZ8lo_qVsxZOvcTI6wpHgL2FvOSxNPyOgtQ17H48t3diik9Kz4HdAcUeytk3JtBXaCOJQ0hAbMEH-0d6g"/>
</div>
</div>
</div>
</header>
...
<nav class="space-y-2">
<a class="flex items-center gap-4 px-4 py-3 text-[#DDC1AE] hover:bg-[#201F1F] rounded-xl hover:translate-x-1 transition-transform active:scale-[0.98]" href="gestion_mesas.html">
<span class="material-symbols-outlined">grid_view</span><span>Distribución de Mesas</span>
</a>
<a class="flex items-center gap-4 px-4 py-3 text-[#DDC1AE] hover:bg-[#201F1F] rounded-xl hover:translate-x-1 transition-transform active:scale-[0.98]" href="toma_pedidos.html">
<span class="material-symbols-outlined">receipt_long</span><span>Pedidos</span>
</a>
<a class="flex items-center gap-4 px-4 py-3 bg-gradient-to-r from-[#FFB77A] to-[#FF8C00] text-[#131313] rounded-xl font-bold active:scale-[0.98]" href="#">
<span class="material-symbols-outlined" style="font-variation-settings: 'FILL' 1;">badge</span><span>Gestión de Personal</span>
</a>
<a class="flex items-center gap-4 px-4 py-3 text-[#DDC1AE] hover:bg-[#201F1F] rounded-xl hover:translate-x-1 transition-transform active:scale-[0.98]" href="facturacion.html">
...
<?php if ($mensaje): ?>
<div class="bg-error/20 text-error p-4 rounded-lg mb-6 border border-error/20"><?php echo htmlspecialchars($mensaje); ?></div>
<?php endif; ?>
...
<tr>
<td colspan="4" class="px-8 py-12 text-center text-on-surface-variant italic">No hay usuarios registrados.</td>
</tr>
<?php else: ?>
...
<td class="px-8 py-5">
<div class="flex items-center gap-4">
<div class="w-12 h-12 rounded-xl bg-surface-container-highest flex items-center justify-center font-bold text-primary bg-gradient-to-br from-primary to-primary-container text-on-primary">
<?php echo getInitials($user['nombre']); ?>
</div>
<div>
<div class="font-bold text-on-surface text-base"><?php echo htmlspecialchars($user['nombre']); ?></div>
<?php if ($user['telefono']): ?>
<div class="text-sm text-on-surface-variant"><?php echo htmlspecialchars($user['email']); ?> • <?php echo htmlspecialchars($user['telefono']); ?></div>
<?php else: ?>
...
<td class="px-6 py-5">
<span class="px-3 py-1 bg-primary/10 text-primary rounded-full text-xs font-bold border border-primary/20 uppercase">
<?php echo htmlspecialchars($user['nombre_rol'] ?? 'Sin asignar'); ?>
</span>
</td>
<td class="px-6 py-5">
<div class="flex items-center gap-2">
<div class="w-2 h-2 rounded-full <?php echo $user['activo'] ? 'bg-secondary animate-pulse' : 'bg-outline'; ?>"></div>
<span class="text-sm font-medium <?php echo $user['activo'] ? 'text-on-surface' : 'text-on-surface-variant'; ?>">
...
<a href="editar_usuario.php?id_usuario=<?php echo (int)$user['id_usuario']; ?>" title="Editar" class="p-2 hover:bg-surface-container-highest rounded-lg transition-all text-on-surface-variant hover:text-primary hover:shadow-md">
<span class="material-symbols-outlined text-lg">edit</span>
</a>
<form method="POST" action="eliminar.php" class="inline" onsubmit="return confirm('¿Eliminar permanentemente a <?php echo addslashes($user['nombre']); ?>?');">
<input type="hidden" name="id_usuario" value="<?php echo (int)$user['id_usuario']; ?>">
<button type="submit" title="Eliminar" class="p-2 hover:bg-error/10 rounded-lg transition-all text-on-surface-variant hover:text-error hover:shadow-md">
<span class="material-symbols-outlined text-lg">person_off</span>
</button>
</form>
<a href="javascript:void(0)" onclick="navigator.clipboard.writeText('<?php echo addslashes($user['email']); ?>')" title="Copiar email" class="p-2 hover:bg-secondary/10 rounded-lg transition-all text-on-surface-variant hover:text-secondary">
...
<!-- FAB -->
<button class="fixed bottom-24 right-6 md:bottom-10 md:right-10 w-16 h-16 bg-gradient-to-br from-[#FFB77A] to-[#FD8E00] text-[#131313] rounded-2xl shadow-[0_12px_32px_rgba(253,142,0,0.4)] flex items-center justify-center active:scale-90 transition-transform z-[60]" onclick="window.location.href='agregar_usuario.php'" title="Nuevo Usuario">
